...
protected:
   ...
   void inOrderMinimumSuccesor(BinaryNode<T> *&sub_root, BinaryNode<T> *&temp);
};
...
*/
{
   if (treeNumberChecker.find(target) != treeNumberChecker.end())
   {
      ...
      if (target < temp->data)
      {
         search_and_delete(sub_root->left, treeNumberChecker, target);
      }
      else if (target > temp->data)
      {
         search_and_delete(sub_root->right, treeNumberChecker, target);
      }
      else if (target == temp->data && sub_root->right != nullptr)
      {
         inOrderMinimumSuccesor(temp, sub_root->right);
      }
   }
   else
   {
      std::cout << "Target is not in Tree, try a different target. \n";
   }
}

template <class T>
void BinarySearchTree<T>::inOrderMinimumSuccesor(BinaryNode<T> *&sub_root, BinaryNode<T> *&temp)
{
   if (temp->left != nullptr)
   {
      inOrderMinimumSuccesor(sub_root, temp->left);
   }
   else
   {
